...
import { buildHeatmapChartOption } from '../../core/builders/heatmap-chart.builder';
import { PuiChartBaseDirective } from '../../core/base/chart-base.directive';
import type { PuiHeatmapChartConfig } from '../../core/interfaces/chart-config.types';
...
@Component({
  selector: 'pui-heatmap-chart',
  template: CHART_SHELL,
  changeDetection: ChangeDetectionStrategy.OnPush,
  host: { class: 'pui-heatmap-chart', '[style.display]': "'block'", '[style.width]': "'100%'" },
})
export class PuiHeatmapChartComponent extends PuiChartBaseDirective {
  readonly config = input<PuiHeatmapChartConfig>();
  readonly height = input(320);

  protected buildOption(data: PuiChartData): PuiInternalEChartsOption {
    return buildHeatmapChartOption(data, this.config(), this.resolveTheme());
  }
